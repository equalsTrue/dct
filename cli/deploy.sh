...
if [ $1 = "prod" ]; then
  ...
  nohup java -server  -Dcom.sun.management.jmxremote.access.file=${SOURCE_PATH}/jmxremote.access -Dcom.sun.management.jmxremote.password.file=${SOURCE_PATH}/jmxremote.password -Djava.rmi.server.hostname=${public_IP} -Dcom.sun.management.jmxremote.rmi.port=6011 -Dcom.sun.management.jmxremote.authenticate=true -Daws.access_key=AKIA2A76QT4MH6TKP3F5 -Daws.secret_access_key=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 -Dcom.sun.management.jmxremote.ssl=false -Dcom.sun.management.jmxremote.port=6011 -jar  $DIR"/../"target/dct-0.0.1-SNAPSHOT.jar --spring.profiles.active=prod --server.port=8080 >/dev/null 2>&1 &
elif [ $1 = "dev" ]; then
